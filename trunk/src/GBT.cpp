...
GBT::GBT(Treedata* treeData, size_t targetIdx, size_t nTrees, num_t shrinkage, num_t subSampleSize):
treeData_(treeData),
...
void GBT::predictForestCategorical(Treedata* treeData, vector<num_t>& categoryPrediction)
{
	...
	for (size_t i=0; i<nSamples; i++)
	{
		...
		errorCnt += (treeData->features_[iTarget].data[i] != categoryPrediction[i]); //// THIS WILL BECOME INVALID UPON REMOVAL OF FRIENDSHIP ASSIGNMENT IN TREEDATA ////
		cout << i << "\t" << treeData->features_[iTarget].data[i] << "\t" << categoryPrediction[i]; //// THIS WILL BECOME INVALID UPON REMOVAL OF FRIENDSHIP ASSIGNMENT IN TREEDATA ////
...
void GBT::predictForestNumerical(Treedata* treeData, vector<num_t>& prediction)
{
	...
	for (size_t i=0; i<nSamples; i++)
	{
		...
		// diagnostic print out the true and the prediction
		cout << i << "\t" << treeData_->features_[iTarget].data[i] << "\t" << prediction[i] <<endl; //// THIS WILL BECOME INVALID UPON REMOVAL OF FRIENDSHIP ASSIGNMENT IN TREEDATA ////
	}
}
...
void GBT::SetNodePrediction( size_t treeIdx, Node* node, vector<size_t>& sampleIcs)
{
	...
	size_t targetIdx = targetIdx_; // OLD: GBT::getTarget();
	//
	...
	if (0==numClasses_) 
	{
		...
		datadefs::mean( data, nodePred,nreal); // ADDED THIS
...
void GBT::recursiveNodeSplit(size_t treeIdx, Node* node, vector<size_t>& sampleIcs)
{
	...
	// exceeding max number of nodes if we split this node? CANNOT HAPPEN, AT LEAST NOT ANYMORE WHEN TREE GENERATION IS DYNAMIC => REMOVED AS OBSOLETE
	/*
	  if ( 2+nNodes_[treeIdx] >= nMaxNodes_) //nNodes_[treeIdx]
	  {
